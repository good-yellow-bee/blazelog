...
func SettingsAlerts(user *session.Session, csrfToken string, cspNonce string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			ctx = templ.InitializeContext(ctx)
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 1, "<div x-data=\"alertsController\" x-init=\"init()\" class=\"space-y-6\"><!-- Header --><div class=\"flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4\"><div><h1 class=\"page-title\">Alert Rules</h1><p class=\"page-subtitle\">Manage alert rules for log monitoring</p></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			if user.Role == "admin" || user.Role == "operator" {
				templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "<button @click=\"openCreate()\" class=\"btn-primary w-auto\"><svg class=\"w-4 h-4\" fill=\"none\" viewBox=\"0 0 24 24\" stroke=\"currentColor\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M12 4v16m8-8H4\"></path></svg> Create Alert</button>")
				if templ_7745c5c3_Err != nil {
					return templ_7745c5c3_Err
				}
			}
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 3, "</div><!-- Filters --><div class=\"panel-soft p-4\"><div class=\"flex flex-col sm:flex-row gap-4\"><div class=\"flex-1\"><label class=\"label\">Search</label> <input type=\"text\" x-model=\"search\" @input.debounce.300ms=\"filterAlerts()\" placeholder=\"Search alerts...\" class=\"input-field\"></div><div class=\"sm:w-48\"><label class=\"label\">Project</label> <select x-model=\"filterProject\" @change=\"loadAlerts()\" class=\"select-field\"><option value=\"\">All Projects</option><template x-for=\"p in projects\" :key=\"p.id\"><option :value=\"p.id\" x-text=\"p.name\"></option></template></select></div><div class=\"sm:w-36\"><label class=\"label\">Type</label> <select x-model=\"filterType\" @change=\"filterAlerts()\" class=\"select-field\"><option value=\"\">All Types</option> <option value=\"pattern\">Pattern</option> <option value=\"threshold\">Threshold</option></select></div></div></div><!-- Alerts Table --><div class=\"table-shell overflow-hidden\"><!-- Loading state --><div x-show=\"loading\" class=\"p-8 text-center\"><svg class=\"animate-spin h-8 w-8 mx-auto text-teal-600\" fill=\"none\" viewBox=\"0 0 24 24\"><circle class=\"opacity-25\" cx=\"12\" cy=\"12\" r=\"10\" stroke=\"currentColor\" stroke-width=\"4\"></circle> <path class=\"opacity-75\" fill=\"currentColor\" d=\"M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z\"></path></svg><p class=\"mt-2 text-slate-500\">Loading alerts...</p></div><!-- Empty state --><div x-show=\"!loading && filteredAlerts.length === 0\" class=\"p-8 text-center\"><svg class=\"mx-auto h-12 w-12 text-slate-400\" fill=\"none\" viewBox=\"0 0 24 24\" stroke=\"currentColor\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9\"></path></svg><h3 class=\"mt-2 text-sm font-semibold text-slate-900\">No alerts found</h3><p class=\"mt-1 text-sm text-slate-500\">Get started by creating a new alert rule.</p></div><!-- Table --><div x-show=\"!loading && filteredAlerts.length > 0\" class=\"overflow-x-auto\"><table class=\"min-w-full divide-y divide-slate-200/70\"><thead class=\"table-head\"><tr><th class=\"px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500\">Name</th><th class=\"px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500\">Type</th><th class=\"px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500\">Severity</th><th class=\"px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500\">Status</th><th class=\"px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500\">Project</th><th class=\"px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500\">Actions</th></tr></thead> <tbody id=\"alerts-tbody\" class=\"divide-y divide-slate-200/70\"><!-- Rows rendered by Alpine.js --></tbody></table></div></div><!-- Create/Edit Modal -->")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			if user.Role == "admin" || user.Role == "operator" {
			...
			templ_7745c5c3_Err = AlertsScript(user.Role, cspNonce).Render(ctx, templ_7745c5c3_Buffer)
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
		})
		templ_7745c5c3_Err = layouts.Base("Alerts", user, csrfToken, cspNonce).Render(templ.WithChildren(ctx, templ_7745c5c3_Var2), templ_7745c5c3_Buffer)
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
	...
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var4 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Var5 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
				...
				ctx = templ.InitializeContext(ctx)
				templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 6, "<div class=\"space-y-4\"><!-- Name --><div><label class=\"label\">Name <span class=\"text-red-500\">*</span></label> <input type=\"text\" x-model=\"form.name\" class=\"input-field\" placeholder=\"Alert name\"><p x-show=\"errors.name\" x-text=\"errors.name\" class=\"mt-1 text-sm text-red-600\"></p></div><!-- Description --><div><label class=\"label\">Description</label> <textarea x-model=\"form.description\" class=\"input-field\" rows=\"2\" placeholder=\"Optional description\"></textarea></div><!-- Type --><div><label class=\"label\">Type <span class=\"text-red-500\">*</span></label> <select x-model=\"form.type\" class=\"select-field\"><option value=\"pattern\">Pattern Match</option> <option value=\"threshold\">Threshold</option></select></div><!-- Condition --><div><label class=\"label\">Condition <span class=\"text-red-500\">*</span></label> <textarea x-model=\"form.condition\" class=\"input-field font-mono text-sm\" rows=\"3\" placeholder='{\"pattern\": \"ERROR|FATAL\"}'></textarea><p class=\"mt-1 text-xs text-slate-500\">JSON condition for the alert rule</p></div><!-- Severity --><div><label class=\"label\">Severity <span class=\"text-red-500\">*</span></label> <select x-model=\"form.severity\" class=\"select-field\"><option value=\"low\">Low</option> <option value=\"medium\">Medium</option> <option value=\"high\">High</option> <option value=\"critical\">Critical</option></select></div><!-- Window and Cooldown --><div class=\"grid grid-cols-2 gap-4\"><div><label class=\"label\">Window</label> <input type=\"text\" x-model=\"form.window\" class=\"input-field\" placeholder=\"5m\"><p class=\"mt-1 text-xs text-slate-500\">e.g., 5m, 1h</p></div><div><label class=\"label\">Cooldown</label> <input type=\"text\" x-model=\"form.cooldown\" class=\"input-field\" placeholder=\"15m\"><p class=\"mt-1 text-xs text-slate-500\">Min time between alerts</p></div></div><!-- Project --><div><label class=\"label\">Project</label> <select x-model=\"form.project_id\" class=\"select-field\"><option value=\"\">No Project (Global)</option><template x-for=\"p in projects\" :key=\"p.id\"><option :value=\"p.id\" x-text=\"p.name\"></option></template></select></div><!-- Notify Channels --><div><label class=\"label\">Notification Channels</label><div class=\"space-y-2\"><label class=\"flex items-center\"><input type=\"checkbox\" x-model=\"form.notify\" value=\"email\" class=\"rounded border-slate-300 text-teal-600 focus:ring-teal-500\"> <span class=\"ml-2 text-sm text-slate-700\">Email</span></label> <label class=\"flex items-center\"><input type=\"checkbox\" x-model=\"form.notify\" value=\"slack\" class=\"rounded border-slate-300 text-teal-600 focus:ring-teal-500\"> <span class=\"ml-2 text-sm text-slate-700\">Slack</span></label> <label class=\"flex items-center\"><input type=\"checkbox\" x-model=\"form.notify\" value=\"teams\" class=\"rounded border-slate-300 text-teal-600 focus:ring-teal-500\"> <span class=\"ml-2 text-sm text-slate-700\">Microsoft Teams</span></label></div></div><!-- Enabled --><div><label class=\"flex items-center\"><input type=\"checkbox\" x-model=\"form.enabled\" class=\"rounded border-slate-300 text-teal-600 focus:ring-teal-500\"> <span class=\"ml-2 text-sm text-slate-700\">Enabled</span></label></div><!-- Error message --><div x-show=\"formError\" class=\"p-3 bg-rose-50 text-rose-700 rounded-md text-sm\" x-text=\"formError\"></div></div>")
				if templ_7745c5c3_Err != nil {
					return templ_7745c5c3_Err
				}
				return nil
...
func AlertsScript(role string, cspNonce string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		templ_7745c5c3_W, ctx := templ_7745c5c3_Input.Writer, templ_7745c5c3_Input.Context
		if templ_7745c5c3_CtxErr := ctx.Err(); templ_7745c5c3_CtxErr != nil {
			return templ_7745c5c3_CtxErr
		...
		if templ_7745c5c3_Err != nil {
			return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/web/templates/pages/settings_alerts.templ`, Line: 278, Col: 25}
		}
		...
		templ_7745c5c3_Var9, templ_7745c5c3_Err := templruntime.ScriptContentInsideStringLiteral(role)
		if templ_7745c5c3_Err != nil {
